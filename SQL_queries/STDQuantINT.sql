COPY INTO REFINITIV.STAGING.STD_QUANT_INT_XML FROM @REFINITIV.STAGING.Stage_STDQuantINT on_error = 'Continue';

CREATE TABLE REFINITIV.STAGING.STD_QUANT_INT_LAYOUT AS
...
FROM REFINITIV.STAGING.STD_QUANT_INT_XML t
...
CREATE TABLE REFINITIV.STAGING.STD_QUANT_INT_FV AS
SELECT GET(XMLGET(XML,'RepNo'),'$')::STRING AS "RepNo"
...
TRUNCATE REFINITIV.PUBLIC.STD_QUANT_INT_DATA;

INSERT INTO REFINITIV.PUBLIC.STD_QUANT_INT_DATA
SELECT   A."RepNo"
		,A."CompanyName"
		,A."CompanyType"
		,A."ActiveStatus"
        ...
        ,B."LineID"
        ,B."Precision"       
FROM REFINITIV.STAGING.STD_QUANT_INT_FV A
INNER JOIN REFINITIV.STAGING.STD_QUANT_INT_LAYOUT B
ON A."RepNo" = B."RepNo" AND 
   A."StatementType" = B."LayoutType" AND
   A."COA" = B."COA";
   
DROP TABLE REFINITIV.STAGING.STD_QUANT_INT_FV;

DROP TABLE REFINITIV.STAGING.STD_QUANT_INT_LAYOUT;
