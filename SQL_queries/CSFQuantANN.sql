COPY INTO REFINITIV.STAGING.CSF_QUANT_ANN_XML FROM @REFINITIV.STAGING.Stage_CSFQuantANN on_error = 'Continue';

CREATE TABLE REFINITIV.STAGING.CSF_QUANT_ANN_LAYOUT AS
...
FROM REFINITIV.STAGING.CSF_QUANT_ANN_XML t
...
CREATE TABLE REFINITIV.STAGING.CSF_QUANT_ANN_FV AS
SELECT GET(XMLGET(XML,'RepNo'),'$')::STRING AS "RepNo"
...
TRUNCATE REFINITIV.PUBLIC.CSF_QUANT_ANN_DATA_NEW;

INSERT INTO REFINITIV.PUBLIC.CSF_QUANT_ANN_DATA_NEW
SELECT
 A."RepNo"
,A."CompanyName"
,A."CompanyType"
...
,B."COA"
,B."Display"
FROM REFINITIV.STAGING.CSF_QUANT_ANN_FV A
INNER JOIN REFINITIV.STAGING.CSF_QUANT_ANN_LAYOUT B
ON A."RepNo" = B."RepNo" AND 
   A."StatementType" = B."LayoutType" AND
   A."LineID" = B."LineID";
   
DROP TABLE REFINITIV.STAGING.CSF_QUANT_ANN_FV;

DROP TABLE REFINITIV.STAGING.CSF_QUANT_ANN_LAYOUT;
