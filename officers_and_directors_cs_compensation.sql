CREATE TABLE REFINITIV.STAGING.OFFICERS_AND_DIRECTORS_CS_COMPENSATION AS
SELECT "RepNo"
     , "OrganizationPermID"
     , "CompanyName"
     , "CompanyNameType"
...
FROM (
         ...
         FROM (
                  ...
                  FROM REFINITIV.STAGING.OFFICERS_AND_DIRECTORS_XML,
                       ...
                       LATERAL FLATTEN(input => CompensationPeriod.value, recursive => true) AS CompanySpecificCompensation,
                       LATERAL FLATTEN(input => CompanySpecificCompensation.value, recursive => true) AS Compensation
                  WHERE GET(officer.value, '@') = 'Officer'
                    AND (L = 4 OR L = 5)
                    AND GET(CompensationPeriod.value, '@') = 'CompensationPeriod'
                    AND GET(CompanySpecificCompensation.value, '@') = 'CompanySpecificCompensation'
