COPY INTO REFINITIV.STAGING.CSF_QUANT_INT_XML FROM @REFINITIV.STAGING.Stage_CSFQuantINT on_error = 'Continue';

CREATE TABLE REFINITIV.STAGING.CSF_QUANT_INT_LAYOUT AS
...
FROM REFINITIV.STAGING.CSF_QUANT_INT_XML t
...
CREATE TABLE REFINITIV.STAGING.CSF_QUANT_INT_FV AS
SELECT GET(XMLGET(XML,'RepNo'),'$')::STRING AS "RepNo"
...
TRUNCATE REFINITIV.PUBLIC.CSF_QUANT_INT_DATA;

INSERT INTO REFINITIV.PUBLIC.CSF_QUANT_INT_DATA
SELECT
     A."RepNo"
    ,A."CompanyName"
    ,A."CompanyType"
    ...
    ,B."COA"
    ,B."Display" 
FROM REFINITIV.STAGING.CSF_QUANT_INT_FV A
INNER JOIN REFINITIV.STAGING.CSF_QUANT_INT_LAYOUT B
ON A."RepNo" = B."RepNo" AND 
   A."StatementType" = B."LayoutType" AND
   A."LineID" = B."LineID";
   
DROP TABLE REFINITIV.STAGING.CSF_QUANT_INT_FV;

DROP TABLE REFINITIV.STAGING.CSF_QUANT_INT_LAYOUT;
